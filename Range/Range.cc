...
int main() {
  ...
  // input - dims
  auto input_start = new npuTensor<float>(ACL_FLOAT,
                                          input_dims.size(),
                                          input_dims.data(),
                                          ACL_FORMAT_NCHW,
                                          input_data_start.data(),
                                          memType::DEVICE);
  auto input_limit = new npuTensor<float>(ACL_FLOAT,
                                          input_dims.size(),
                                          input_dims.data(),
                                          ACL_FORMAT_NCHW,
                                          input_data_limit.data(),
                                          memType::DEVICE);
  auto input_delta = new npuTensor<float>(ACL_FLOAT,
                                          input_dims.size(),
                                          input_dims.data(),
                                          ACL_FORMAT_NCHW,
                                          input_data_delta.data(),
                                          memType::DEVICE);
  ...
  // output - out
  auto output = new npuTensor<float>(ACL_FLOAT,
                                     output_dims.size(),
                                     output_dims.data(),
                                     ACL_FORMAT_NCHW,
                                     nullptr);
