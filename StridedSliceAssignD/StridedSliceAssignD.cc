...
int main() {
  ...
  // inputs
  auto input_var = new npuTensor<float>(ACL_FLOAT,
                                        var_dims.size(),
                                        var_dims.data(),
                                        ACL_FORMAT_ND,
                                        var_data.data());
  auto input_value = new npuTensor<float>(ACL_FLOAT,
                                          value_dims.size(),
                                          value_dims.data(),
                                          ACL_FORMAT_ND,
                                          value_data.data());
  // auto input_begin = new npuTensor<int64_t>(ACL_INT64, begin_dims.size(),
  // begin_dims.data(), ACL_FORMAT_ND, begin_data.data()); auto input_end = new
  // npuTensor<int64_t>(ACL_INT64, end_dims.size(), end_dims.data(),
  // ACL_FORMAT_ND, end_data.data()); auto input_stride = new
  // npuTensor<int64_t>(ACL_INT64, stride_dims.size(), stride_dims.data(),
  // ACL_FORMAT_ND, stride_data.data()); set inputs desc and buffer
  ...
  ACL_CALL(
      aclopSetAttrListInt(attr, "strides", strides.size(), strides.data()));
