...
int main() {
  ...
  // ACL_CALL(aclSetTensorOriginShape(x_desc, x_origin_dims.size(),
  // x_origin_dims.data()));
  auto x_size = std::accumulate(x_storage_dims.begin(),
                                x_storage_dims.end(),
                                1,
                                std::multiplies<int64_t>()) *
                4;  // aclGetTensorDescSize(x_desc);
  ...
  // ACL_CALL(aclSetTensorOriginShape(y_desc, y_origin_dims.size(),
  // y_origin_dims.data()));
  auto y_size = std::accumulate(y_storage_dims.begin(),
                                y_storage_dims.end(),
                                1,
                                std::multiplies<int64_t>()) *
                4;  // aclGetTensorDescSize(y_desc);
  ...
  // ACL_CALL(aclSetTensorOriginShape(out_desc, x_origin_dims.size(),
  // x_origin_dims.data()));
  auto out_size = std::accumulate(x_storage_dims.begin(),
                                  x_storage_dims.end(),
                                  1,
                                  std::multiplies<int64_t>()) *
                  4;  // aclGetTensorDescSize(out_desc);
