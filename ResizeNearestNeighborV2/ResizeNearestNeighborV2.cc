...
int main() {
  ...
  ACL_CALL(aclrtMemcpy(
      x_host_ptr, x_size, x.data(), x_size, ACL_MEMCPY_HOST_TO_HOST));
  ...
  ACL_CALL(aclrtMemcpy(
      x_device_ptr, x_size, x.data(), x_size, ACL_MEMCPY_HOST_TO_DEVICE));
  auto x_device_buffer = aclCreateDataBuffer(x_device_ptr, x_size);

  // input1 - sizes - should use host buffer
  auto sizes_desc = aclCreateTensorDesc(
      ACL_INT32, sizes_dims.size(), sizes_dims.data(), ACL_FORMAT_NCHW);
  ...
  ACL_CALL(aclrtMemcpy(sizes_host_ptr,
                       sizes_size,
                       sizes.data(),
                       sizes_size,
  ...
  ACL_CALL(
      aclrtMalloc(&sizes_device_ptr, sizes_size, ACL_MEM_MALLOC_NORMAL_ONLY));
  ACL_CALL(aclrtMemcpy(sizes_device_ptr,
                       sizes_size,
                       sizes.data(),
                       sizes_size,
  ...
  auto y_desc = aclCreateTensorDesc(
      ACL_FLOAT, y_dims.size(), y_dims.data(), ACL_FORMAT_NCHW);
  ...
  ACL_CALL(aclrtMemcpy(
      y.data(), y_size, y_device_ptr, y_size, ACL_MEMCPY_DEVICE_TO_HOST));
